#!/bin/sh
# FasterSeedbox for Debian/Ubuntu
# POSIX-compliant, production-hardened system optimizer for high-throughput seedboxes
#
# Usage: ./debian.sh [--dry-run] [--help]
#
# Fixes applied:
#   - P0: tcp_adv_win_scale now injected into sysctl
...
#   - P1: --dry-run now previews actual content
#   - P1: systemctl daemon-reexec -> daemon-reload
#   - P2: lsblk word-splitting fixed
#   - P2: verify_sysctl handles permission denied
...
readonly SCRIPT_VERSION="1.1.0-hardened"
...
Debian/Ubuntu system optimizer for high-throughput BitTorrent/PT seeding.

Options:
  --dry-run    Preview changes without modifying system
  --help       Show this help message and exit

Supported: Debian 10+, Ubuntu 18.04+
EOF
    exit 0
}
...
generate_sysctl_conf() {
    cat <<EOF
# FasterSeedbox Network Stack Optimization
...
generate_limits_conf() {
    cat <<EOF
# FasterSeedbox File Descriptor Limits
# Managed by $SCRIPT_NAME v$SCRIPT_VERSION
$MARKER
...
generate_systemd_conf() {
    cat <<EOF
# FasterSeedbox systemd Resource Limits
# Managed by $SCRIPT_NAME v$SCRIPT_VERSION
# Applies to all systemd services
$MARKER
...
================================================================================
FasterSeedbox for Debian/Ubuntu - Configuration Summary
================================================================================
Applied:
  ✓ $CONF_DIR/99-seedbox.conf
  ✓ $LIMITS_DIR/99-seedbox.conf
  ✓ $SYSTEMD_CONF_DIR/99-seedbox.conf
  ✓ $MODULES_CONF_DIR/seedbox-bbr.conf
  ✓ $RUNTIME_BIN
  ✓ $RUNTIME_SERVICE

System: $(. /etc/os-release 2>/dev/null && printf '%s %s' "$NAME" "$VERSION_ID" || echo 'Debian/Ubuntu')
Kernel: $(uname -r)
...
main() {
    log_info "FasterSeedbox for Debian/Ubuntu v$SCRIPT_VERSION"
    check_root
    detect_virt
    calc_memory_tier
