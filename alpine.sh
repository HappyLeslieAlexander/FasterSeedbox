...
#   - P1: OpenRC service replaces systemd (rc.d + rc_ulimit)
...
readonly SCRIPT_VERSION="1.1.0-hardened-alpine"
readonly SYSCTL_DIR="/etc/sysctl.d"
readonly LIMITS_DIR="/etc/security/limits.d"
readonly MODULES_CONF="/etc/modules"
readonly INIT_D="/etc/init.d"
readonly CONF_D="/etc/conf.d"
...
Alpine Linux system optimizer for high-throughput BitTorrent/PT seeding.

Options:
  --dry-run    Preview changes without modifying system
  --help       Show this help message and exit

Supported: Alpine 3.14+ (musl libc, OpenRC)
EOF
    exit 0
}
...
check_root() {
    if [ "$(id -u)" -ne 0 ]; then
        die "This script must be run as root (use doas/sudo)"
    fi
}
...
detect_virt() {
    ...
    if command -v virt-what >/dev/null 2>&1; then
        _virt="$(virt-what 2>/dev/null | head -1)"
...
# Apply ulimits to the service process
rc_ulimit="-n 1048576"

start() {
    ebegin "Starting FasterSeedbox runtime tuning"
    /usr/local/sbin/seedbox-runtime.sh
    eend $?
}

stop() {
    ebegin "Stopping FasterSeedbox runtime tuning"
    # No daemon to stop; settings persist until reboot/sysctl reset
    eend 0
}

status() {
    einfo "FasterSeedbox runtime tuning is applied via sysctl/rc"
    einfo "Check: sysctl net.ipv4.tcp_congestion_control"
}
OPENRCEOF
}
...
tune_nic() {
    ...
    if command -v ethtool >/dev/null 2>&1; then
        # Safe multi-line parser for ethtool -g
...
apply_sysctl() {
    ...
    _sysctl_path="${SYSCTL_DIR}/99-seedbox.conf"
    if write_file "$_sysctl_path" <<EOF
$(generate_sysctl_conf)
EOF
    then
        if [ $DRY_RUN -eq 0 ]; then
            sysctl -p "$_sysctl_path" 2>/dev/null || log_warn "sysctl apply failed (may require reboot)"
        fi
    fi
}
...
apply_openrc() {
    log_info "Installing OpenRC service & runtime helper..."
    mkdir -p "$INIT_D" "$CONF_D" 2>/dev/null || true
    
    write_file "$INIT_D/seedbox-tune" <<EOF
$(generate_openrc_init)
EOF
    if [ $DRY_RUN -eq 0 ]; then
        chmod 755 "$INIT_D/seedbox-tune" 2>/dev/null || true
        rc-update add seedbox-tune default 2>/dev/null || log_warn "rc-update failed"
    fi
...
================================================================================
FasterSeedbox for Alpine Linux - Configuration Summary
================================================================================
Applied:
  ✓ $SYSCTL_DIR/99-seedbox.conf    (sysctl network parameters)
  ✓ $LIMITS_DIR/99-seedbox.conf    (PAM limits)
  ✓ $MODULES_CONF                  (kernel modules)
  ✓ $INIT_D/seedbox-tune           (OpenRC boot service)
  ✓ $RUNTIME_BIN                   (runtime tuning)

System: Alpine $(cat /etc/alpine-release 2>/dev/null || echo 'unknown')
Kernel: $(uname -r)
Memory: $((TOTAL_MEM_KB / 1024)) MB | Virtualization: $VIRT_KIND

Next steps:
  1. Start service: rc-service seedbox-tune start
  2. Verify BBR: sysctl net.ipv4.tcp_congestion_control
  3. Test throughput: iperf3 -c <server> -t 30

Rollback:
  rc-service seedbox-tune stop
  rc-update del seedbox-tune default
  rm -f $INIT_D/seedbox-tune $RUNTIME_BIN \
        $SYSCTL_DIR/99-seedbox.conf $LIMITS_DIR/99-seedbox.conf
  # Remove tcp_bbr from $MODULES_CONF if manually appended
  sysctl --system
...
main() {
    log_info "FasterSeedbox for Alpine Linux v$SCRIPT_VERSION"
    check_root
    detect_virt
    calc_memory_tier
